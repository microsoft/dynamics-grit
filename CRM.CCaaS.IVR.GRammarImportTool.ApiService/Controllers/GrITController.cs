...
public class GrITController() : ControllerBase
{
        ...
        IOptions<GptChatGrxmlConfiguration> gptPrompterConfiguration)
    {
        ...
        _logger.LogInformation("Received ZIP file upload request. FileName={FileName}, Size={FileSizeBytes}", file.FileName, file.Length);
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Starting results dequeue task. ChannelCapacity={ChannelCapacity}", gptPrompterConfiguration.Value.ResultStreamChannelCapacity);
                try
                {
                    ...
                    {
                        _logger.LogInformation("Processed entry. Key={Key}, Timestamp={Timestamp}", partialResult.Key, DateTime.UtcNow);
            ...
            await Response.Body.FlushAsync();
            _logger.LogInformation("File processing completed. Result={Result}, DurationMs={Duration}", conversionResult, stopwatch.ElapsedMilliseconds);

            await deQueueTask;
        }
        catch (OperationCanceledException ex)
        {
            _logger.LogError(ex, "Cancellation exception. FileName={FileName}", file.FileName);
            SetContentType(Response, "text/plain");
        ...
        {
            _logger.LogError(ex, "An error occurred while processing file. FileName={FileName}, Size={FileSizeBytes}", file.FileName, file.Length);
            SetContentType(Response, "text/plain");
        ...
        IOptions<GptChatGrxmlConfiguration> gptPrompterConfiguration)
    {
        ...
        _logger.LogInformation("Received Grxml file upload request. FileName={FileName}, Size={FileSizeBytes}", file.FileName, file.Length);
        ...
        try
        {
            ...
            {
                _logger.LogError("Conversion returned empty result. FileName={FileName}, ContentLength={Length}", file.FileName, grxmlContent?.Length ?? 0);
                WriteErrorResponse(Response, $"Conversion failed: {converted}", HttpStatusCode.InternalServerError);
                return;
            }
            if (HttpContext.RequestAborted.IsCancellationRequested)
            {
                _logger.LogInformation("Request was cancelled. FileName={FileName}", file.FileName);
                return;
            }
            SetContentType(Response, "application/x-yaml");
            _logger.LogInformation("Conversion success. FileName={FileName}, OutputLength={Length}", file.FileName, converted?.Length ?? 0);
            Response.StatusCode = StatusCodes.Status200OK;
            await Response.WriteAsync($"---\n#{file.FileName}\n{SerializeYaml(converted)}");
        }
        catch (OperationCanceledException ex)
        {
            _logger.LogError(ex, "Cancellation exception while processing GRXML file. FileName={FileName}", file.FileName);
            SetContentType(Response, "text/plain");
            WriteErrorResponse(Response, "File processing was cancelled.", HttpStatusCode.RequestTimeout);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error occurred while processing GRXML file. FileName={FileName}", file.FileName);
            SetContentType(Response, "text/plain");
